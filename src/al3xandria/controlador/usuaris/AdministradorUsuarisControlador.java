...
public class AdministradorUsuarisControlador implements MouseListener {
	 ...
	 */
	public AdministradorUsuarisControlador(
			AdministradorUsuaris administradorUsuaris, Usuari usuariConnectat) {
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (administradorUsuaris.getMostrarButton() == e.getSource()
		...
		if (administradorUsuaris.getAltaUsuariButton() == e.getSource()
				&& getAccio().equals("default")) {
			setAccio("alta");
		...
		if (administradorUsuaris.getBaixaUsuariButton() == e.getSource()
				&& getAccio().equals("default")) {
			if (comprovarSiUsuariSeleccionat()) {
				setAccio("baixa");
		...
		if (administradorUsuaris.getEditarUsuariButton() == e.getSource()
				&& getAccio().equals("default")) {
			if (comprovarSiUsuariSeleccionat()) {
				setAccio("editar");
	...
	/**
	 * Mostra un avís per modificar un usuari Comproba que totes les dades
	...
	private void avisConfirmarUpdateUsuari() {
		if (comprovacioDadesAltaUsuari()) {
			...
			int valor = JOptionPane.showConfirmDialog(administradorUsuaris,
					...
					"Dades l'usuari modificat", JOptionPane.YES_NO_OPTION,
					JOptionPane.WARNING_MESSAGE, null);
			if (valor == JOptionPane.YES_OPTION) {
				if (enviarDadesModificarUsuariAlServidor()) {
					mostraDadesUsuariModificat(dadesUsuariModificat,
							dadesUsuariToString());
					refrescarElsUsuaris();
					setPanelPerDefecte();
				} else {
	...
	private boolean enviarDadesModificarUsuariAlServidor() {
		...
		comunicacioClientServidor
				.iniciarComunicacio(usuariConnectat.getIdSessio()
						+ ",modificar_usuari," + dadesUsuariToString());
	...
	private void mostraErrorModificacioUsuari() {
		JOptionPane.showMessageDialog(administradorUsuaris,
				"No s'ha pogut modificar l'usuari", "Error modificació",
				JOptionPane.ERROR_MESSAGE);

	}

	/**
	 * Mostra un avís amb l'informació de l'usuari que s'ha modificat amb les
	 ...
	 * @author SergioHernandez
	 */
	private void mostraDadesUsuariModificat(String dadesUsuariModificat,
			Object dadesUsuariModificatToString) {
		String idUsuari = getIdUsuari();
		JOptionPane.showMessageDialog(administradorUsuaris,
				"Has modificat un usuari amb aquestes dades: \n"
						+ dadesUsuariModificat
						+ "\nDades enviades al servidor: "
						+ usuariConnectat.getIdSessio() + ",modificar_usuari,"
	...
	private String dadesUsuariToString() {
		String dadesUsuariModificat = administradorUsuaris.getNomField()
				.getText() + ","
				...
				+ administradorUsuaris.getTipusUsuariComboBox()
	...
	private String dadesUsuariModificat() {
		String dadesUsuariModificat = "id: "
				+ administradorUsuaris.getIdUsuariField().getText() + "\n "
	...
	private void avisConfirmarAltaUsuari() {
		...
		if (comprovacioDadesAltaUsuari()) {
			...
			int valor = JOptionPane.showConfirmDialog(administradorUsuaris,
					...
					"Dades de l'usuari a afegir", JOptionPane.YES_NO_OPTION,
					JOptionPane.WARNING_MESSAGE, null);
			if (valor == JOptionPane.YES_OPTION) {
				if (enviarDadesAltaUsuariAlServidor()) {
					// mostraDadesUsuariAlta(dadesUsuariAlta,
					// dadesUsuariToString());
					refrescarElsUsuaris();
					setPanelPerDefecte();
				} else {
	...
	/**
	 * Envia les dades al servidor per executar la consulta per afegir el usuari
	 * 
	 * @return 1 si tot ok, 0 si no s'ha pogut fer
	 * @author SergioHernandez
	 */
	private boolean enviarDadesAltaUsuariAlServidor() {
		...
		comunicacioClientServidor
				.iniciarComunicacio(usuariConnectat.getIdSessio()
						+ ",insercio_usuari," + dadesUsuariToString());
		String rebutString = comunicacioClientServidor.getData();
		if (rebutString.equals("1")) {
			return true;
		} else {
	...
	/**
	 * Comprova que totes les dades introduïdes per fer l'alta d'un usuari
	 * siguin correctes. Camps buits, email i telefon correctes
	 * 
	...
	private boolean comprovacioDadesAltaUsuari() {
		...
		if (controlDeDades.comprovarCampsOmplertsAltaUsuari(
				...
				administradorUsuaris.getTipusUsuariComboBox()
						.getSelectedIndex())) {
			if (controlDeDades.comprovacioEmail(
					administradorUsuaris.getEmailField().getText())) {

				if (controlDeDades.comprovacioFormatTelefon(
						administradorUsuaris.getTelefonField().getText())) {
	 ...
	 * @author SergioHernandez
	 */
	private void mostraDadesUsuariAlta(String dadesUsuariAlta,
			Object dadesUsuariAltaToString) {
		JOptionPane.showMessageDialog(administradorUsuaris,
				"Has afegit un usuari amb aquestes dades: \n" + dadesUsuariAlta
						+ "\nDades enviades al servidor: "
						+ usuariConnectat.getIdSessio() + ",insercio_usuari,"
						+ dadesUsuariAltaToString,
	...
	/**
	 * Dades de l'usuari per donar d'alta per mostrar en el missatge
	 * d'informació
	 * 
	 * @return dades de l'usuari amb salts de linia
	 * @author SergioHernandez
	...
	private void avisConfirmarBaixaUsuari() {
		int rowAMostrar = administradorUsuaris.getUsuarisTable()
				.getSelectedRow();
		administradorUsuaris.setRowActiu(
				administradorUsuaris.getUsuarisTable().getSelectedRow());
		setAccio("default");
		String dadesUsuariAEliminar = getDadesRowLUsuari(
				administradorUsuaris.getRowActiu());
		int valor = JOptionPane.showConfirmDialog(administradorUsuaris,
				dadesUsuariAEliminar + "\n\n" + "Vols eliminar l'usuari?",
				"Dades del usuari a eliminar", JOptionPane.YES_NO_OPTION,
				JOptionPane.WARNING_MESSAGE, null);
		if (valor == JOptionPane.YES_OPTION) {
			if (enviarDadesUsuariAEliminarAlServidor()) {
				mostraDadesUsuariAEliminar(dadesUsuariAEliminar,
						getDadesRowToString(
								administradorUsuaris.getRowActiu()));
	...
	private boolean enviarDadesUsuariAEliminarAlServidor() {
		...
		comunicacioClientServidor.iniciarComunicacio(
				usuariConnectat.getIdSessio() + ",borrar_usuari," + carnet);
		String rebutString = comunicacioClientServidor.getData();
		if (rebutString.equals("1")) {
			return true;
		} else {
	...
	private void mostraErrorEliminarUsuari() {
		JOptionPane.showMessageDialog(administradorUsuaris,
				"No s'ha pogut eliminar l'usuari", "Error eliminació",
				JOptionPane.ERROR_MESSAGE);
	 ...
	 * @author SergioHernandez
	 */
	private void mostraDadesUsuariAEliminar(String dadesUsuariAEliminar,
		...
		JOptionPane.showMessageDialog(administradorUsuaris,
				"Has esborrat un usuari amb aquestes dades: \n"
						+ dadesUsuariAEliminar
						+ "\nDades enviades al servidor: "
						+ usuariConnectat.getIdSessio() + ",eliminar_usuari,"
	...
	private String getDadesRowToString(int row) {
		String dadesRowToString = getValorCella(row, 0) + ","
				...
				+ getValorCella(row, 13) + "," + getValorCella(row, 14) + ","
	...
	private void getDadesRow(int row) {
		administradorUsuaris.getRowActualField().setText(
				String.valueOf(administradorUsuaris.getRowActiu() + 1));
		administradorUsuaris.getRowTotalsField()
				.setText(String.valueOf(table.getRowCount()));
		administradorUsuaris.getUsuarisTable().setRowSelectionInterval(row,
				row);
		...
		administradorUsuaris.getContrasenyaField()
				.setText(getValorCella(row, 5));
		administradorUsuaris.getAdrecaField().setText(getValorCella(row, 6));
		administradorUsuaris.getCodiPostalField()
				.setText(getValorCella(row, 7));
		administradorUsuaris.getPoblacioField().setText(getValorCella(row, 8));
		administradorUsuaris.getProvinciaComboBox()
				.setSelectedItem(getValorCella(row, 9));
		...
		administradorUsuaris.getTipusUsuariComboBox()
				.setSelectedItem(getValorCella(row, 13));
		administradorUsuaris.getPuntuacioField()
				.setText(getValorCella(row, 14));
	...
	private void errorUsuariNoSeleccionat() {
		JOptionPane.showMessageDialog(administradorUsuaris,
				"No has seleccionat cap usuari",
				WarningStrings.getString("Error selecció usuari"),
				JOptionPane.ERROR_MESSAGE);
	...
	private void ferLaCerca() {
		...
		String tipusUsuariString = administradorUsuaris.getFiltreComboBox()
				.getSelectedItem().toString().toLowerCase();
...
//							+ ",consulta_usuari_" + filtre + "," + tipusUsuariString + "," + textDeLaCerca);
	...
	/**
	 * Mostra un missatge d'error si el text de cerca no és númeric
	 */
	private void errorCampCercaCarnetIncorrecta() {
		JOptionPane.showMessageDialog(administradorUsuaris,
				"El camp de cerca per carnet ha de ser numéric",
				"Error cerca carnet", JOptionPane.ERROR_MESSAGE);
	...
	private void mostraLaCerca(String cerca) {
		JOptionPane.showMessageDialog(administradorUsuaris, cerca,
	...
	private String[] getFiltre() {
		...
		if (grup.getSelection()
				.equals(administradorUsuaris.getNomRadioButton().getModel())) {
			filtre[0] = "nom";
		}
		if (grup.getSelection().equals(
				administradorUsuaris.getCarnetRadioButton().getModel())) {
			filtre[0] = "carnet";
		}
		if (grup.getSelection().equals(
				administradorUsuaris.getDniNieRadioButton().getModel())) {
			filtre[0] = "dni/nie";
		}
	...
	public void mousePressed(MouseEvent e) {
		// TODO Auto-generated method stub
		if (administradorUsuaris.getMostrarContrasenyaLabel() == e
				.getSource()) {
