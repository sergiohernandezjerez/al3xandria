...
public class PrestecsModel {
	...
	private String[] titols = { "Id", "llibre", "usuari", "data inici",
			"data final", "num. renovacio" };
	...
	public DefaultTableModel consultarTotsElsPrestecs() {
		...
		try {
			...
			comunicacioClientServidor
					.iniciarComunicacio("0000000000000,consulta_prestecs");
			String dadesDelServidor = comunicacioClientServidor.getData();
			Gson gson = new Gson();
			llistatDePrestecs = new ArrayList<Prestecs>();
			Prestecs[] llistat = gson.fromJson(dadesDelServidor,
					Prestecs[].class);
	 ...
	 * @return
	 */
	public DefaultTableModel consultarPrestecsUsuari(String idSessio,
			int idUsuari) {
		...
		try {
			...
			comunicacioClientServidor.iniciarComunicacio(
					idSessio + ",consulta_prestecs_usuari," + idUsuari);
			String dadesDelServidor = comunicacioClientServidor.getData();
			Gson gson = new Gson();
			llistatDePrestecs = new ArrayList<Prestecs>();
			Prestecs[] llistat = gson.fromJson(dadesDelServidor,
